...
router.get("/user/:userId/plants", async (req, res) => {
  ...
  const plantLocationName: string | undefined = req.query.location as string | undefined;
  try {
...
const validatePlantLocation = async (locationName: string): Promise<PlantLocation> => {
  const plantLocation: PlantLocation = (await queryGetPlantLocationByLocationName(locationName)) as PlantLocation;
...
const checkIfUserHasPlantsOfLocation = (allPlantsOfUserOfLocation: Plant[]) => {
  if (allPlantsOfUserOfLocation.length === 0) throw new ConflictError(errorMessages.userHasNoPlantsOfLocation);
};
